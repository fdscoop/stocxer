#!/usr/bin/env python3
"""
Live NIFTY Scan - Production API with Extended Timeout
Shows real-time scanning process with all logs
"""
...
print("=" * 70)
print("🔴 LIVE NIFTY SCAN - Real-Time Market Analysis")
print("=" * 70)
...
print()
print("=" * 70)
print("🔍 LIVE SCAN IN PROGRESS...")
print("=" * 70)
print("📊 Scanning all 50 NIFTY constituent stocks")
...
try:
    ...
    if response.status_code == 200:
        ...
        print(f"📅 Data timestamp: {result.get('timestamp', 'N/A')}")
        print()
        
        # Display results (same format as before)
        prediction = result.get("prediction", {})
        print("=" * 70)
        print("📈 LIVE INDEX PREDICTION")
        print("=" * 70)
        ...
        if bullish:
            print("=" * 70)
            print("🚀 TOP 5 BULLISH STOCKS (LIVE)")
            print("=" * 70)
        ...
        if bearish:
            print("=" * 70)
            print("📉 TOP 5 BEARISH STOCKS (LIVE)")
            print("=" * 70)
        ...
        if option_rec:
            print("=" * 70)
            print("💡 LIVE OPTIONS RECOMMENDATION")
            print("=" * 70)
            ...
            print(f"Trade Setup:")
            print(f"  Target: {option_rec.get('expected_target', 0):,.2f}")
        ...
        # Save
        filename = f"nifty_live_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print("   The scan is taking longer than expected.")
    print("   This can happen during market hours with high load.")
except Exception as e:
    print(f"❌ Error: {e}")
    import traceback
    traceback.print_exc()
